#ifndef STRUTTURE_DATI_INSERTION_SORT_HPP
#define STRUTTURE_DATI_INSERTION_SORT_HPP
...
/*
 * l'algoritmo di ordinamento "insertion sort" è di tipo in loco (sul posto o in-place) cioe che ha una complessita spaziale di O(1), questo
 * per che è un algoritmo che usa una quantita costante di memoria per la sua impementazione (come alcune variabili ausiliari) a prescindere dell'input.
 ...
 * 3. è sensibile all’ordinamento dell’input, cioe se trova un elemento maggiore dell’array già ordinato, lo capisce e non va ad eseguire il ciclo interno.
 *
 * svantaggi:
 * la complessita dell'algoritmo è O(n) nel caso migliore (quando l'input è gia ordinato), ma O(n2) quando l'input è ordinato in ordine inverso, per cio la complessita computazionale sarebbe O(n2)
 */

// detto questo di solito l'insertion sort viene usato come algoritmo per ordinare input di piccole dimensioni o come algoritmo ausiliario, altre opzioni come il merge_ sort o quick sort potrebbero essere piu efficienti
...
#endif //STRUTTURE_DATI_INSERTION_SORT_HPP